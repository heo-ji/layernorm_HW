...
//	);
    ...
    case (c_state)
        ...
        ITER2: begin
            if (w_norm_done && m00_axis_tlast) //check!!if (m00_axis_tvalid && m00_axis_tready && m00_axis_tlast)??
                n_state = IDLE; 
        end
    ...
    //assign s00_axis_tready  = w_calc_ready || w_norm_ready;
    // ─── AXI-Stream slave ready (output) 
    assign s00_axis_tready = (c_state == ITER1) ? w_calc_ready :
                             (c_state == ITER2) ? w_norm_ready :
                               1'b0;
                         
    assign m00_axis_tvalid = (c_state == ITER2) && w_norm_done;
    assign m00_axis_tlast  = s00_axis_tlast & (c_state == ITER2);
    assign m00_axis_tstrb  = {(512/8){1'b1}}; //m_axis_data all byte is active
