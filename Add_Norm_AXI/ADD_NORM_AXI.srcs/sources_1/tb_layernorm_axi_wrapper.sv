...
module tb_layernorm_axi_wrapper;
...
task send_768_beats(input [511:0] base_data);
    ...
    $display("[AXIS SEND] with continuous valid : all beats send 완료");
endtask
...
task send_768_beats_with_validgap(input [511:0] base_data);
    ...
    $display("[AXIS SEND] with input valid gap : all beats send 완료");
endtask
...
initial begin
    ...
    //axis_slv_agent.vif_proxy.set_ready_policy(XIL_AXI_READY_GEN_RANDOM);//axis-slave가 tready=0을랜덤으로
...
always @(dut.inst.c_state) begin
    case (dut.inst.c_state)
        ...
        default:$display("[DUT_FSM] t=%0t ns  UNKNOWN=%b",  $time, dut.inst.c_state);
    endcase
end
...
// always @(posedge aclk) begin
//     if (dut.inst.c_state == 3'b100 && iter2_start_cycle == 0)
