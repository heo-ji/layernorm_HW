`timescale 1ns/1ps
/*
128bit BUS = 8-row(TOKEN) concurrent process

MODEL_DIMENSION=768

/trace128 folder txt file = [8,768]data가 col-wise로 저장

=128-bit hex x 768 line
= 한줄에 8개 16bit 데이터
...
logic [127:0] input_mem    [0:MODEL_DIMENSION-1];
logic [127:0] rtl_norm_mem [0:MODEL_DIMENSION-1];
logic [127:0] axi_norm_mem [0:MODEL_DIMENSION-1];
...
begin
    input_file    = "trace128/in_data.txt";
    rtl_norm_file = "trace128/rtl_norm.txt";
...
begin
    axi_norm_file = "trace128/AXI_norm.txt";
...
begin
    ...
    for (i = 0; i < MODEL_DIMENSION; i = i + 1) begin
        if (axi_norm_mem[i] !== rtl_norm_mem[i]) begin
            ...
            $error("[COMPARE ERROR] beat=%0d actual_AXI=%032x expected_RTL=%032x",
...
always @(posedge aclk) begin
    if (axis_out_tvalid && axis_out_tready) begin // 실제 transfer된 beat만 로그 출력 , TVALID/TREADY handshake 정상? 을 체크
        $display("[AXIS OUT_DATA] first row data[15:0] in 8row = %h , tlast = %b",
...
//
// 저장 format:
// 128-bit 한 줄 = 32 hex digit
// 왼쪽 hex = row7, 오른쪽 hex = row0
//─────────────────────────────────────────────
always @(posedge aclk) begin
    if (!aresetn) begin
        out_cnt            <= 0;
