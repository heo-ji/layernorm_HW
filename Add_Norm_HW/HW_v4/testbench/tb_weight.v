...
reg signed [DATA_WIDTH-1 : 0] i_weight;
...
initial begin
    ...
    file = $fopen("/home/linuxusr/code/mpw/MPW_2024/Add_Norm/HW/testbench/768norm.txt", "r");
    if (file == 0) begin
        $display("Error: failed to open input file.");
        $finish;
    end

    file2 = $fopen("/home/linuxusr/code/mpw/MPW_2024/Add_Norm/HW/testbench/768weight.txt", "r");
    if (file2 == 0) begin
        $display("Error: failed to open file2 file.");
        $finish;
    end

    fp = $fopen("/home/linuxusr/code/mpw/MPW_2024/Add_Norm/HW/testbench/768weightedout.txt", "r");
    if (fp == 0) begin
...
always @(posedge clk) begin
   ...
   begin
        ...
        if (o_data !== ref) no_difference = 1'b0; //software data 와 HW data(o_data)와의 비교
