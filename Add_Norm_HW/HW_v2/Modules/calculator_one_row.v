...
module calculator_one_row #(
    ...
    input                                   i_s_valid,
    output                                  o_s_ready,//o_s_ready=1 & _s_valid=1 = accumulate start
    output                                  o_m_valid,//done signal
    input                                   i_m_ready,
...
begin
    ...
    begin
        if(o_s_ready) begin //o_m_valid=1 뜨고 다음 clock에 값 업데이트됨
...
always @( posedge i_clk , negedge i_rstn) // update next state
begin
...
assign o_s_ready = (i_m_ready || ~o_m_valid)&&(c_state == IDLE || c_state == CALC_ACCUM);//1'b1; //i_m_ready || ~o_m_valid; //+ IDLE,ACCUMa
